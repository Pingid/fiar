...
import { format, AstPath } from 'prettier'
import { print } from '../printer'
import { rule, output } from '../rule/index.js'
import { transformRule } from './rules.js'

const match = async (schema: FireSchemaTypes) => {
  // const lll = transformRule(
  //   rule(() => ({ kind: 'Ident', name: 'data' })),
  //   schema,
  // )
  // const k = new AstPath(output({ kind: 'Ident', name: 'data' }, lll))
  // console.log({ k })
  // console.log(output({ kind: 'Ident', name: 'data' }, lll))
  // const base = output(
...
describe('string', () => {
  // console.log(match(s.string()))
  // it('assert', async () => (await match(s.string())).toBe('data is string'))
