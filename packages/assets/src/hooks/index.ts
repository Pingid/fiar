...
import { useConfig } from '../context/index.js'
...
export const useRemoveAsset = (path: string) => {
  const storage = useStorage()

  return useMutaton(path.split('/').slice(0, -1).join('/'), () => deleteObject(ref(storage, path)), {
    optimisticData: (x: any) => {
      return { ...x, items: x.items.filter((y: any) => y.fullPath !== path) }
    },
    revalidate: true,
