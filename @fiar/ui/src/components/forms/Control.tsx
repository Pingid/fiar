  ...
  return (
    <div className={cn('relative transition-[padding]', [!!error, 'pb-6'], [!!label, 'pt-6'])}>
      ...
      {error && (
        <label
          htmlFor={name}
          className="group-focus-within:text-error text-error/80 absolute -bottom-0 right-2 text-sm"
        >
          {error}
        </label>
      )}
      <Element
        ...
        ref={ref}
        className={cn(
          'focus-within:border-active border-front/5 bg-front/5 group w-full w-full rounded-sm border',
          props.className,
        )}
      >
        {children}
